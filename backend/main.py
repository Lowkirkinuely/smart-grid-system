...
from test_runner import IntegrationTester
...
async def run_integration_tests():
    """Run integration tests from frontend and return results"""
    test_id = str(uuid.uuid4())

    logger.info(f"[TEST] Starting integration tests | test_id: {test_id}")
    ...
    try:
        ...
        logger.info(f"[TEST] Completed | passed: {summary['passed']}/{summary['total']}")
...
async def websocket_tests(websocket: WebSocket):
    ...
    try:
        ...
        tests = [
            ("Grid State Validation", tester.test_grid_state_validation()),
            ("API Endpoint", tester.test_api_endpoint(None)),
            ("WebSocket Connection", tester.test_websocket_connection()),
            ("Operator Intent", tester.test_operator_intent_message()),
            ("Apply Plan", tester.test_apply_plan_message()),
        ]

        passed = 0
        for idx, (test_name, test_coro) in enumerate(tests):
            try:
                ...
                await websocket.send_json({
                    ...
                    "progress_percent": int((idx + 1) / len(tests) * 100)
                })

                logger.info(f"[TEST] {test_name}: {'✓ PASS' if result.passed else '✗ FAIL'}")
